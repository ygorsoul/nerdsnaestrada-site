...
- **Fonts** — already set to **Playfair Display** (display ramp) / **Inter** (reading ramp), exactly as the /alamo page uses them; ignore any preset font name lingering in prose.
...
- **Colors** — the frontmatter is the /alamo palette: cream canvas `#FAF8F3`, ink `#332C20`, Álamo orange `#EF6C1F` as the accent (eyebrows, labels, active state), taupe `#C8BFAE` hairlines, navy `#14477F` reserved for the Nerds na Estrada mark. Use the frontmatter hex; preset color NAMES in prose (e.g. "cobalt", "cream") mean the remapped brand values.
